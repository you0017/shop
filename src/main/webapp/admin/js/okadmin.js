...
layui.use(["element", "form", "layer", "okUtils", "okTab", "okLayer", "okContextMenu", "okHoliday", "laydate"], function () {
    ...
    // okHoliday.getContent()
    function noticeFun() {
        var srcWidth = okUtils.getBodyWidth();
        layer.open({
            type: 0, title: "系统公告", btn: "开始使用", btnAlign: 'c', content: okHoliday.getContent(),
            yes: function (index) {
                if (srcWidth > 800) {
    ...
    $("#logout").click(function () {
        okLayer.confirm("确定要退出吗？", function () {
            okUtils.ajax("login.action?op=logout", "post", null, true).done(function (response) {
                okLayer.greenTickMsg("正在退出", function () {
    ...
    form.on("submit(login)", function (data) {
        okUtils.ajax("../login.action?op=logout", "post", data.field, true).done(function (response) {
